...
for tabsidx=1:length(glmb_update.tt)
    [mtemp_predict,Ptemp_predict]= ekf_predict_multiple(model,glmb_update.tt{tabsidx}.m,glmb_update.tt{tabsidx}.P);         %ekf prediction for GM
...
for emm= 1:m
    for tabidx= 1:length(glmb_predict.tt)
        ...
        [qz_temp,m_temp,P_temp] = ekf_update_multiple(meas.Z{k}(:,emm),model,glmb_predict.tt{tabidx}.m,glmb_predict.tt{tabidx}.P);      %ekf update for this track and this measurement
