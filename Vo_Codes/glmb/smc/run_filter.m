...
filter.H_sur= 5000;                 %requested number of surviving components/hypotheses
...
filter.npt= 10000;                   %number of particles per track
filter.nth= 5000;                   %threshold on effective number of particles before resampling
...
for tabsidx=1:length(glmb_update.tt)
    wtemp_predict= compute_pS(model,glmb_update.tt{tabsidx}.x).*glmb_update.tt{tabsidx}.w(:); xtemp_predict= gen_newstate_fn(model,glmb_update.tt{tabsidx}.x,'noise');      %particle prediction
