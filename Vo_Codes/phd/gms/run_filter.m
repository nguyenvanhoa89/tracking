...
for k=1:meas.K
    ...
    w_predict= model.P_S*w_update;   % w_update = w_{k-1};                                          %surviving weights
    ...
    w_update = model.Q_D*w_predict; %w_{k} missed for detection
    m_update = m_predict; % m_{k} for missed detection
    P_update = P_predict; % P_{k|k-1} for missed detection
    
    if m~=0
        %m detection terms 
        [qz_temp,m_temp,P_temp] = kalman_update_multiple(meas.Z{k},model,m_predict,P_predict);
        for ell=1:m
            w_temp = model.P_D*w_predict(:).*qz_temp(:,ell); % w_{k}(z) numerator
            w_temp= w_temp./(model.lambda_c*model.pdf_c + sum(w_temp)); %w_{k}(z)
            w_update = cat(1,w_update,w_temp); %w_{k}
            m_update = cat(2,m_update,m_temp(:,:,ell)); %m_{k}
            P_update = cat(3,P_update,P_temp); % P_{k}
        end
    end         
