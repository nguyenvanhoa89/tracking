...
for k=1:meas.K
    ...
    for j=0:filter.N_max % original: N_max is infinity, current: cardinality ditribution n from 0 to N_max
        idxj=j+1;
        terms= zeros(filter.N_max+1,1);
        for ell=j:filter.N_max % original: l = j to infinity
            idxl= ell+1;
            terms(idxl) = exp(sum(log(1:ell))-sum(log(1:j))-sum(log(1:ell-j))+j*log(model.P_S)+(ell-j)*log(model.Q_S))*cdn_update(idxl); % cdn_update(idxl) = \rho_{k-1}(l)
        end
        survive_cdn_predict(idxj) = sum(terms); % survive cdn of n where n from 0 to N_max
    end
    ...
    for n=0:filter.N_max
        ...
        for j=0:n
            ...
            % birth distribution: p(j) = (1/j!)*e^(sum(w_birth))*(sum(w_birth)): Poisson distribution
        end
    ...
    for ell=1:m
       XI_vals(ell) = model.P_D*w_predict'*qz_temp(:,ell)/model.pdf_c; %model.pdf_c: uniform clutter density
    end
    ...
    upsilon0_E = zeros(filter.N_max+1,1); % Z_{k}
    upsilon1_E = zeros(filter.N_max+1,1); % Z_{k}
    upsilon1_D = zeros(filter.N_max+1,m); % Z_{k}\{z}
    
    for n=0:filter.N_max % original, card distribution for n is from 0 to infinity, to reduce computational expense,set n from 0 to N_max only
        idxn= n+1; % index n
        ...
        if m~= 0                        %calculate upsilon1_D(idxn,:) if m>0 % Z_{k}\{z}
    ...
    %--- state extraction
    [~,idx_max_cdn] = max(cdn_update); % use Maximum A Posterior (MAP) instead of Expected A Posterior (EAP)
