...
for k=1:meas.K
    ...
    for t=1:T_update
        ...
        [m_predict{t},P_predict{t}] = ekf_predict_multiple(model,m_update{t},P_update{t});                       %surviving components
    ...
    %---gating
    if filter.gate_flag
        meas.Z{k}= gate_meas_ekf(meas.Z{k},filter.gamma,model,m_pseudo,P_pseudo);        
    end
    ...
    if m~=0
        offset= T_predict;
        [qz_temp,m_temp,P_temp] = ekf_update_multiple(meas.Z{k},model,m_pseudo,P_pseudo);
        for ell=1:m                                        
